...
router.put("/:bookingId", requireAuth, async (req, res, next) => {
  ...
  for (let alreadyBookedSpot of allBookingsOfThisSpot) {
    //the try catch block cannot be in the for loop
    console.log(
      "abs",
      alreadyBookedSpot.id,
      "desbi",
      desiredBookingId,
      "sd",
      startDate,
      "absSD",
      alreadyBookedSpot.startDate,
      "ed",
      endDate,
      "absED",
      alreadyBookedSpot.endDate
    );
...
router.delete("/:bookingId", requireAuth, async (req, res, next) => {
  ...
  if (!currentBooking) {
    ...
    currentBooking.startDate < currentDateTotal &&
    currentBooking.endDate > currentDateTotal
  ) {
    res.statusCode = 403;
    res.json({
