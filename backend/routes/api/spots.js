...
router.get("/", async (req, res, next) => {
  ...
  if (searchTerm) {
    (mySearchTerm = searchTerm), console.log("mySearchTermNow", mySearchTerm);
  }
  if (age === "oneDay") {
    sortDate = new Date(Date.now() - 60 * 60 * 1000 * 24);
  }
  ...
  try {
    if (page && Number.isNaN(page)) {
      errObj["page"] = errorStrings["page"];
    }
    if (size && Number.isNaN(size)) {
      errObj["size"] = errorStrings["size"];
    }
    if (minLat && Number.isNaN(minLat)) {
      errObj["minLat"] = errorStrings["minLat"];
    }
    if (maxLat && Number.isNaN(maxLat)) {
      errObj["maxLat"] = errorStrings["maxLat"];
    }
    if (minLng && !(minLng > -180)) {
      errObj["minLng"] = errorStrings["minLng"];
    }
    if (maxLng && !(maxLng < 180)) {
      errObj["maxLng"] = errorStrings["maxLng"];
    }
    if (minPrice && !(minPrice > 0)) {
      errObj["minPrice"] = errorStrings["minPrice"];
    }
    // if( maxPrice && Number.isNaN(maxPrice)) {errObj['maxPrice'] = errorStrings['maxPrice']}
    if (priceRange && Number.isNaN(priceRange[0])) {
      errObj["maxPrice"] = errorStrings["maxPrice"];
    }
    page = parseInt(page);
    size = parseInt(size);
...
router.put("/:spotId", requireAuth, async (req, res, next) => {
  ...
  try {
    if (!address) {
      errObj["address"] = errorStrings["address"];
    }
    if (!city) {
      errObj["city"] = errorStrings["city"];
    }
    if (!state) {
      errObj["state"] = errorStrings["state"];
    }
    if (!country) {
      errObj["country"] = errorStrings["country"];
    }
    // if(!lat) {errObj['lat'] = errorStrings['lat']}
    // if(!lng) {errObj['lng'] = errorStrings['lng']}
    if (!name) {
      errObj["name"] = errorStrings["name"];
    }
    if (!description) {
      errObj["description"] = errorStrings["description"];
    }
    if (!price) {
      errObj["price"] = errorStrings["price"];
    }
    ...
    const newSpot = await updatedSpot.update({
      address,
      city,
      state,
      country,
...
router.post("/:spotId/bookings", requireAuth, async (req, res, next) => {
  //in postman its spotIdForBooking

  const spotIdForBooking = req.params.spotId;
  const userId = req.user.id;
  let { startDate, endDate } = req.body;
  console.log(
    "XXXXXXXXXXXXXXXXXXXXXXX entering the start of create booking 21?",
    startDate,
    endDate
  );
  ...
  const errorStrings2 = {
    //I don't have this set up for them to work independently
  ...
  for (let alreadyBookedSpot of allBookingsOfThisSpot) {
    if (startDate > endDate) {
      //before & after or equal a booking
      ...
      startDate <= alreadyBookedSpot.startDate &&
      endDate >= alreadyBookedSpot.endDate
    ) {
      //before & after or equal a booking
      ...
      startDate <= alreadyBookedSpot.startDate &&
      endDate > alreadyBookedSpot.startDate
    ) {
      ...
      endDate >= alreadyBookedSpot.endDate &&
      startDate < alreadyBookedSpot.endDate
    ) {
  ...
  try {
    console.log(
      "XXXX///XXXXX???XXXX???entering backend try block?",
      spotIdForBooking,
      userId,
      startDate,
      endDate
