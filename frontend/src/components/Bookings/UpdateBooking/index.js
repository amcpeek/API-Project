...
const UpdateBookingForm = ({ showModal, setShowModal, sendBookingId }) => {
  ...
  const allBookings = useSelector((state) => {
  ...
  if (allBookings) {
    currentBooking = allBookings.find(
      (booking) => booking.id == sendBookingId.bookingId
    );
  }

  const newToday = new Date();

  const todayFormatted = new Intl.DateTimeFormat("default", {
    dateStyle: "full",
  }).format(newToday);
  const options = {
    year: "numeric",
  ...
  if (arrayDay[1].length == 1) {
    arrayDay[1] = `0${arrayDay[1]}`;
  }
  if (arrayDay[0].length == 1) {
    arrayDay[0] = `0${arrayDay[0]}`;
  }
  ...
  if (arrayTom[1].length == 1) {
    arrayTom[1] = `0${arrayTom[1]}`;
  }
  if (arrayTom[0].length == 1) {
    arrayTom[0] = `0${arrayTom[0]}`;
  }
  ...
  const handleSubmit = async (e) => {
    ...
    const response = await dispatch(
      updateSpotBooking(newBooking, sendBookingId.bookingId)
    );
      ...
      <div className="realModalContent">
        <div className="outerFormTop">
          <div className="formTop">
            ...
            {currentBooking && (
              <h3 className="nowrapBookings">
                Update Dates for "{currentBooking.Spot.name}"
              </h3>
            )}
