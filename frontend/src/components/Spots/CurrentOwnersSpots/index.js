...
const CurrentOwnersSpots = () => {
  ...
  let futureBookings = [];
  if (usersBookings) {
    futureBookings = usersBookings.filter(
      (book) => new Date(book.startDate) >= new Date()
    );
  }
  //past trips
  let pastBookings = [];
  if (usersBookings) {
    pastBookings = usersBookings.filter(
      (book) => new Date(book.startDate) < new Date()
    );
  }
  ...
  let futureGuests = [];
  if (ownersBookings) {
    futureGuests = ownersBookings.filter(
      (book) => new Date(book.startDate) >= new Date()
    );
  }
  //past guests
  let pastGuests = [];
  if (ownersBookings) {
    pastGuests = ownersBookings.filter(
      (book) => new Date(book.startDate) < new Date()
    );
  }
  ...
  return (
    <div className="CurrentOwnersPage">
      {userFirstName && (
        <h1 className="currentOwnersTitle">{userFirstName}'s Profile</h1>
      )}
      <h2 className="yourTripTitle">Future Trips</h2>
      <div className="outerYourTrips">
        <div className="yourTrips">
          {futureBookings?.map(
            ({ id, spotId, userId, startDate, endDate, Spot }) => (
              <div key={id} className="oneTrip col">
                ...
                <div className="row">
                  <div>
                    <UpdateBookingModal bookingId={id} />
                  </div>
                  <button
      ...
      <div className="outerYourTrips">
        <div className="yourTrips">
          {futureGuests?.map(
            ({ id, spotId, userId, startDate, endDate, Spot, Guest }) => (
              <NavLink to={`/spots/${spotId}`} key={spotId}>
                <div key={id} className="oneTrip">
                  <img
                    className="smallImg"
            ...
            <NavLink to={`/spots/${review.spotId}`} key={review.id}>
              <div key={review.id} className="insideCurrentOwner">
                <div className="SingleSpotReviewBox2" key={review.id}>
                  <div className="underlined shouldWrap">
                    {review.Spot.name}
                  </div>
      ...
      <div className="CurrentOwnersReviews">
        {reviewsAboutCurrent &&
          reviewsAboutCurrent?.map((review) => (
            <NavLink to={`/spots/${review.spotId}`} key={review.id}>
              <div key={review.id} className="insideCurrentOwner">
                <div className="SingleSpotReviewBox2" key={review.id}>
                  <div className="underlined shouldWrap">
