...
export const addSpotImageAction = (image) => {
  //example doesn't have spotImageId
  return {
    type: ADD_SPOT_IMAGE,
    image,
  };
...
export const addSpot = (spot) => async (dispatch) => {
  const response = await csrfFetch(`/api/spots`, {
    ...
    .then(async (res) => {
      //above 400, that thing is a res, send it
      return await res.json();
    })
    .catch(async (error) => {
      //below 400, that thing is an error, send it
...
export const updateSpot = (spot) => async (dispatch) => {
  try {
    ...
    if (response.ok) {
      // as written, will not have errors you can read, it says 'readable stream"
  ...
  switch (action.type) {
    case GET_SPOTS:
      ...
      action.spots.forEach((spot) => {
        //wont work if spots isn't an array
